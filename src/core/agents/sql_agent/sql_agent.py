...
def _build_payload(state: GraphState, task: str, dependent_results: Dict[str, str]) -> str:
    """Формирует JSON-контекст для ReAct SQL-агента."""
...
class SQLAgent:
    ...
    def process_state(self, state: GraphState) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                "ThreadID: %s: SQLAgent завершён (шаг: %s, result_len=%d, tools=%d)",
            ...
            logger.error(
                "ThreadID: %s: SQLAgent ошибка (шаг: %s): %s",
